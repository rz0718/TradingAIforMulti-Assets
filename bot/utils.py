...
STATE_COLUMNS = [
    ...
    'num_positions', 'position_details', 'total_margin', 'net_unrealized_pnl'
]
...
def init_csv_files() -> None:
    ...
    files_to_init = {
        ...
        DECISIONS_CSV: ['timestamp', 'coin', 'signal', 'reasoning', 'confidence'],
...
def log_trade(trade_data: Dict[str, Any]) -> None:
    ...
    header = ['timestamp', 'coin', 'action', 'side', 'quantity', 'price',
              'profit_target', 'stop_loss', 'leverage', 'confidence',
              'pnl', 'balance_after', 'reason']
...
def log_ai_decision(decision_data: Dict[str, Any]) -> None:
    ...
    header = ['timestamp', 'coin', 'signal', 'reasoning', 'confidence']
